...
<body class="bg-gray-100 h-screen overflow-hidden flex text-gray-800">
    ...
    <div class="w-full md:w-1/3 bg-white shadow-2xl flex flex-col h-full border-l border-gray-200 z-20 relative">
        ...
        <div class="p-5 bg-gray-50 border-t border-gray-200 shadow-[0_-5px_20px_rgba(0,0,0,0.05)] flex-shrink-0 z-30 relative">
            ...
            <form action="{{ route('transaksi.store') }}" method="POST" id="checkout-form">
                @csrf
                <input type="hidden" name="total_bayar_hidden" id="total-bayar-input">
    ...
    <script>
        // Ambil data keranjang dari Session PHP
        const rawCartSession = <?php echo json_encode(session('cart', [])); ?>;

        // Pastikan format data array valid
        ...
        // Formatter Mata Uang (Rp 100.000)
        function formatRupiah(number) {
            return new Intl.NumberFormat('id-ID').format(number);
        }

        // Formatter Input saat mengetik (Menambah titik otomatis)
        ...
        function calculateTotal() {
            ...
            if (currentPaymentMethod === 'qris') {
                // Saat QRIS, input bayar disamakan dengan total, tapi formatnya harus string agar lolos regex
                document.getElementById('uang-bayar').value = formatRupiah(totalPrice);
            }
            checkPayment();
        }
        ...
        function togglePayment(method) {
            ...
            if (method === 'qris') {
                ...
                inputUang.value = formatRupiah(total); 
            } else {
        ...
        // Logic Hitung Kembalian & Validasi Tombol
        function checkPayment() {
            let total = parseInt(document.getElementById('total-bayar-input').value) || 0;

            // PERBAIKAN: Gunakan regex untuk mengambil hanya angka (menghapus titik/koma/huruf)
            let rawUang = document.getElementById('uang-bayar').value;
            let cleanUang = rawUang.replace(/[^0-9]/g, ''); 
            let uang = parseInt(cleanUang) || 0;

            let kembalian = uang - total;

            // Tampilkan kembalian
            if (uang > 0 && total > 0) {
                // Math.max agar tidak minus saat ditampilkan
            ...
            // Tombol aktif jika: ada barang di keranjang DAN uang cukup
